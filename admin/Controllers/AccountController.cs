...
namespace admin.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
         using (ApuDbContext dataContext = new ApuDbContext())
          {
              var AccountList = dataContext.Accounts.ToList();
        ...
        public JsonResult getAccountByID(string AccountID)
      {
          using (ApuDbContext dataContext = new ApuDbContext())
          {
              ...
              var AccountList = dataContext.Accounts.Find(AccID);
              return Json(AccountList, JsonRequestBehavior.AllowGet);
          }
      }
      public string UpdateAccount(Account Acc)
      {
          if (Acc != null)
          {
              using (ApuDbContext dataContext = new ApuDbContext())
              {
                  ...
                  var AccObj = dataContext.Accounts.Where(x => x.AccountID == AccID).FirstOrDefault();
                  AccObj.Image = Acc.Image;
                  AccObj.Fullname = Acc.Fullname;
                  AccObj.Username = Acc.Username;
                  AccObj.Password = Acc.Password;
                  AccObj.Sex = Acc.Sex;
                  AccObj.DateOfBirth = Acc.DateOfBirth;
                  AccObj.Address = Acc.Address;
      ...
      public string DeleteAccount(Account Acc)
      {
          if (Acc != null)
          {
              using (ApuDbContext dataContext = new ApuDbContext())
              {
                  int no = Convert.ToInt32(Acc.AccountID);
                  var AccountList = dataContext.Accounts.Where(x => x.AccountID == no).FirstOrDefault();
                  dataContext.Accounts.Remove(AccountList);
                  dataContext.SaveChanges();
